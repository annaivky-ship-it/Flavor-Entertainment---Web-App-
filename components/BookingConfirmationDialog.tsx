...
const BookingConfirmationDialog: React.FC<BookingConfirmationDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="card-base !p-0 !bg-zinc-950 border-zinc-800/50 max-w-2xl w-full flex flex-col max-h-[90vh] shadow-2xl shadow-black/50 overflow-hidden">
        <div className="flex-shrink-0 p-6 flex justify-between items-center border-b border-zinc-800/50 bg-zinc-900/20">
          <h2 className="text-2xl font-bold text-white tracking-tight">Please Confirm Your Booking</h2>
          <button onClick={onClose} disabled={isLoading} className="text-zinc-500 hover:text-white transition-colors">
            <X className="h-6 w-6" />
          </button>
        </div>

        <div className="flex-grow p-6 sm:p-8 overflow-y-auto space-y-6 bg-gradient-to-b from-zinc-950 to-black">
            <p className="text-zinc-400 text-sm italic">Please review the details below before submitting your request. This is your final chance to make changes.</p>
        ...
        <div className="flex-shrink-0 p-4 bg-zinc-900/40 border-t border-zinc-800/50 flex flex-col sm:flex-row justify-end items-center gap-4">
