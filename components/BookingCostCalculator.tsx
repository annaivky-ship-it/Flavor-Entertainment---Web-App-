...
const BookingCostCalculator: React.FC<BookingCostCalculatorProps> = ({
  ...
  const { totalCost, depositAmount } = useMemo(() => {
    return calculateBookingCost(serviceDurations, performers.length);
  }, [serviceDurations, performers.length]);

  const { formattedTotalDuration } = useMemo(() => {
    return getBookingDurationInfo(serviceDurations);
  }, [serviceDurations]);

  const durationWarning = useMemo(() => {
    const selectedServices = Object.keys(serviceDurations);
    ...
    for (const service of hourlyServices) {
      const duration = serviceDurations[service.id] || 0;
    ...
    // Check total duration against performer minimums
    const totalDurationHours = Object.entries(serviceDurations).reduce((sum, [id, dur]) => {
        const s = allServices.find(srv => srv.id === id);
        if (s?.rate_type === 'per_hour') return sum + dur;
        return sum;
    }, 0);

    for (const performer of performers) {
      if (performer.min_booking_duration_hours && totalDurationHours < performer.min_booking_duration_hours) {
        return `Warning: ${performer.name} requires a minimum total booking duration of ${performer.min_booking_duration_hours} hours.`;
      }
    }

    return null;
  }, [serviceDurations, performers]);
