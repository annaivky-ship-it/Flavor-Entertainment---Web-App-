...
const BookingCostCalculator: React.FC<BookingCostCalculatorProps> = ({
  ...
  return (
    <div className={`card-base !p-6 !bg-[#0f0f12]/50 border-[#2a2a35]/50 transition-all duration-500 ${className} ${totalCost > 0 ? 'ring-1 ring-[#e6398a]/30 shadow-[0_0_20px_rgba(249,115,22,0.1)]' : ''}`}>
      <div className="flex justify-between items-center mb-6">
        ...
        {onClearAll && totalCost > 0 && (
          <button 
            onClick={onClearAll}
            className="text-xs font-semibold text-[#8888a0] hover:text-red-400 transition-colors flex items-center gap-1 px-2 py-1 rounded bg-[#13131a] hover:bg-red-400/10 border border-[#2a2a35]"
          >
            Clear
          </button>
        )}
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-end">
          <span className="text-sm text-[#b8b8c2]">Total Booking Cost</span>
      ...
      {formattedTotalDuration !== 'N/A' && (
        <div className="mt-6 pt-6 border-t border-[#2a2a35]/50">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2 text-[#b8b8c2]">
              <Clock size={16} className="text-[#e6398a]/70" />
              <span className="text-sm">Total Duration</span>
            </div>
            <span className="font-bold text-white">{formattedTotalDuration}</span>
          </div>
        </div>
      )}
      ...
      {totalCost === 0 && (
        <p className="mt-4 text-[10px] text-[#8888a0] text-center italic">
