...
import { Camera, Upload, CheckCircle, ArrowRight, User, Tag, Sparkles, LoaderCircle, X } from 'lucide-react';
...
const PerformerOnboarding: React.FC<PerformerOnboardingProps> = ({ onSubmit, onCancel }) => {
  ...
  const generatePlaceholder = () => {
    ...
    // Simulate a brief loading state for "generation" feel
    setTimeout(() => {
      setFormData(prev => ({ ...prev, photo_url: url }));
        ...
        <div className="card-base !p-8 space-y-6 animate-fade-in">
          ...
          <div className="space-y-4 pt-4 border-t border-zinc-800">
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              <div className="w-32 h-40 rounded-xl border border-zinc-800 bg-zinc-900/50 overflow-hidden flex-shrink-0 relative group">
                {formData.photo_url ? (
                  <>
                    <img src={formData.photo_url} alt="Preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    <button 
                      type="button"
              ...
              <div className="flex-1 space-y-3 w-full">
                <div className="flex flex-wrap gap-3">
                  <label className="btn-secondary !py-2 !px-4 cursor-pointer flex items-center gap-2 text-xs">
                ...
                <p className="text-[10px] text-zinc-500 italic leading-relaxed">
                  High-quality photos increase your booking rate by up to 300%. <br />
                  Admin can generate a placeholder if you don't have one ready.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button 
              type="button" 
              onClick={handleNext}
              disabled={!formData.name || !formData.tagline || !formData.bio || !formData.photo_url}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {SERVICE_AREAS.map(area => (
                <label key={area} className="flex items-center gap-3 p-3 rounded-xl border border-zinc-800 hover:border-zinc-700 bg-zinc-900/50 cursor-pointer transition-colors">
                  <input
                    type="checkbox"
                    checked={formData.service_areas.includes(area as any)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setFormData({ ...formData, service_areas: [...formData.service_areas, area as any] });
                      } else {
