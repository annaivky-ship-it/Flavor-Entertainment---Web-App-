...
const Toggle = ({ label, description, enabled, onChange }: { label: string, description: string, enabled: boolean, onChange: (e: boolean) => void}) => (
    <div 
        ...
        className="flex justify-between items-center p-4 bg-[#1a1a22]/50 rounded-lg cursor-pointer border border-[#2a2a35] hover:bg-[#2a2a35]/70 transition-colors"
    >
        <div>
            <p className="font-semibold text-white">{label}</p>
            <p className="text-sm text-[#b8b8c2]">{description}</p>
        </div>
        <div className={`w-14 h-8 flex items-center rounded-full p-1 transition-colors ${enabled ? 'bg-[#e6398a]' : 'bg-[#2a2a35]'}`}>
...
const UserSettings: React.FC<UserSettingsProps> = ({ settings, onSettingsChange, onBack }) => {
    ...
    return (
        <div className="animate-fade-in max-w-2xl mx-auto">
            <button onClick={onBack} className="mb-8 bg-[#1a1a22] hover:bg-[#2a2a35] text-white font-semibold px-6 py-2 rounded-lg transition-colors duration-300 flex items-center gap-2">
                <ArrowLeft className="h-5 w-5" />
                Back to Gallery
            </button>
            <div className="card-base !p-8">
                <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3"><Bell /> Notification Settings</h1>
                <p className="text-[#b8b8c2] mb-8">Choose which communications you'd like to receive from us.</p>
