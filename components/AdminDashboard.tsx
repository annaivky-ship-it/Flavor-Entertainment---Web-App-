...
const statusClasses: Record<BookingStatus, string> = {
    ...
    completed: 'border-[#8888a0]/50 bg-[#13131a]/30 text-[#b8b8c2]',
    cancelled: 'border-[#8888a0]/50 bg-[#13131a]/30 text-[#b8b8c2]',
  ...
  return (
    <div className="animate-fade-in space-y-8">
        ...
        <div className="flex flex-wrap gap-2">
          {(import.meta.env.DEV || isDemoMode) && (
            <button 
              ...
              className="bg-[#1a1a22] hover:bg-[#2a2a35] text-white font-semibold px-5 py-2.5 rounded-lg transition-colors duration-300 flex items-center justify-center gap-2 border border-[#2a2a35]"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="card-base !p-6 flex items-center gap-4"><BarChart className="w-10 h-10 text-[#e6398a]" /><div><p className="text-[#b8b8c2] text-sm">Total Bookings</p><p className="text-3xl font-bold text-white">{totalBookings}</p></div></div>
        <div className="card-base !p-6 flex items-center gap-4"><ShieldCheck className="w-10 h-10 text-green-500" /><div><p className="text-[#b8b8c2] text-sm">Confirmed</p><p className="text-3xl font-bold text-white">{confirmedBookingsCount}</p></div></div>
        <div className="card-base !p-6 flex items-center gap-4"><ShieldAlert className="w-10 h-10 text-yellow-500" /><div><p className="text-[#b8b8c2] text-sm">Pending Actions</p><p className="text-3xl font-bold text-white">{pendingDnsEntries.length + pendingBookings}</p></div></div>
      </div>

      {/* Tab Navigation */}
      <div className="border-b border-[#2a2a35] overflow-x-auto scrollbar-hide">
        <nav className="-mb-px flex space-x-8 min-w-max px-2" aria-label="Tabs">
          <button
            onClick={() => setActiveTab('management')}
            className={`${activeTab === 'management' ? 'border-[#e6398a] text-[#e6398a]' : 'border-transparent text-[#b8b8c2] hover:text-white hover:border-[#8888a0]'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2`}
          >
            <CreditCard size={16}/> Management
          </button>
          <button
            onClick={() => setActiveTab('payments')}
            className={`${activeTab === 'payments' ? 'border-[#e6398a] text-[#e6398a]' : 'border-transparent text-[#b8b8c2] hover:text-white hover:border-[#8888a0]'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2`}
          >
            <DollarSign size={16}/> Payments
          </button>
          <button
            onClick={() => setActiveTab('performers')}
            className={`${activeTab === 'performers' ? 'border-[#e6398a] text-[#e6398a]' : 'border-transparent text-[#b8b8c2] hover:text-white hover:border-[#8888a0]'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2`}
          >
            <UserCog size={16}/> Performers
          </button>
          <button
            onClick={() => setActiveTab('dns')}
            className={`${activeTab === 'dns' ? 'border-[#e6398a] text-[#e6398a]' : 'border-transparent text-[#b8b8c2] hover:text-white hover:border-[#8888a0]'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2`}
          >
          ...
          <button
            ...
            className={`${activeTab === 'reporting' ? 'border-[#e6398a] text-[#e6398a]' : 'border-transparent text-[#b8b8c2] hover:text-white hover:border-[#8888a0]'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2`}
          >
      ...
      {(activeTab === 'management' || activeTab === 'payments') && (
        <div className="flex flex-col xl:flex-row justify-between xl:items-center gap-4 bg-[#13131a]/40 p-4 rounded-xl border border-[#2a2a35] animate-fade-in">
          <div className="flex flex-wrap items-center gap-3">
             <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#b8b8c2] pointer-events-none" />
                <input
                    type="text"
             ...
             <div className="relative flex items-center">
                <ArrowUpDown className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#b8b8c2] pointer-events-none" />
                <select
                    value={sortField}
                ...
                <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 text-[#b8b8c2] pointer-events-none" />
                
                <button 
                    onClick={() => setSortDirection(prev => prev === 'asc' ? 'desc' : 'asc')}
                    className="ml-2 p-2 bg-[#1a1a22] border border-[#2a2a35] rounded-md text-[#b8b8c2] hover:text-white hover:border-[#8888a0] transition-colors"
             ...
             <div className="relative">
                <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#b8b8c2]" />
                <select
                    value={statusFilter}
            ...
            <div className="card-base !p-6 border-[#e6398a]/30 bg-[#e6398a]/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm text-[#b8b8c2]">Name</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-sm text-[#b8b8c2]">Tagline</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1 md:col-span-2">
                  <label className="text-sm text-[#b8b8c2]">Bio</label>
                  <textarea 
                ...
                <div className="space-y-1">
                  <label className="text-sm text-[#b8b8c2]">Photo URL</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-sm text-[#b8b8c2]">Status</label>
                  <select 
                ...
                <div className="space-y-1">
                  <label className="text-sm text-[#b8b8c2]">Min Booking Duration (Hours)</label>
                  <input 
                    type="number" 
                    min="1"
                    step="0.5"
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button 
                  ...
                  className="px-4 py-2 text-[#b8b8c2] hover:text-white transition-colors"
                >
                  Cancel
                </button>
                <button 
                  ...
                  {performers.filter(p => p.status === 'pending_verification').map(p => (
                    <div key={p.id} className="card-base !p-4 flex flex-col gap-4 border-yellow-500/30 bg-yellow-500/5">
                      <div className="flex gap-4">
                        <img src={p.photo_url} alt={p.name} loading="lazy" className="w-20 h-20 rounded-lg object-cover border border-[#2a2a35]" />
                        <div className="flex-grow">
                          <h4 className="font-bold text-white">{p.name}</h4>
                          <p className="text-xs text-[#b8b8c2] line-clamp-2 mt-1">{p.bio}</p>
                ...
                <div className="flex-grow">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-white">{p.name}</h4>
                      <div className="flex items-center gap-1 text-[10px] bg-[#1a1a22] px-1.5 py-0.5 rounded border border-white/5">
                    ...
                    <div className="flex gap-1">
                      <button 
                        ...
                        className="p-1 text-[#b8b8c2] hover:text-[#e6398a] transition-colors"
                      >
                        <Edit size={16} />
                      </button>
                      <button 
                        ...
                        className="p-1 text-[#b8b8c2] hover:text-red-500 transition-colors"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${p.status === 'available' ? 'bg-green-500/20 text-green-400' : 'bg-[#1a1a22] text-[#8888a0]'}`}>
                      {p.status}
                    </span>
            ...
            <div className="card-base !p-6 !border-yellow-500/50 !bg-yellow-900/10">
              ...
              <div className="space-y-4">
                {pendingDnsEntries.map(entry => (
                  <div key={entry.id} className="bg-[#0f0f12]/50 p-5 rounded-xl border border-[#2a2a35] flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-[#b8b8c2]">
                        ...
                        <span className="mx-2 text-[#8888a0]">|</span>
                        {new Date(entry.created_at).toLocaleDateString()}
                      </p>
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#8888a0] mt-2">
                      ...
                      <p className="text-sm text-[#b8b8c2] mt-3 p-3 bg-[#13131a]/50 rounded-lg border border-[#2a2a35]/50 italic">
                        "{entry.reason}"
                      </p>
                    </div>
                    <div className="flex flex-row md:flex-col gap-2 flex-shrink-0">
                      ...
                      <button 
                        ...
                        disabled={loadingState?.id === entry.id} 
                        className="bg-[#1a1a22] hover:bg-[#2a2a35] text-[#b8b8c2] font-bold py-2 px-4 rounded-lg flex items-center gap-2 text-sm transition-all hover:scale-[1.02] active:scale-[0.98]"
                      >
            ...
            <div className="space-y-4">
              {doNotServeList.filter(e => e.status === 'approved' && (
                ...
                doNotServeList.filter(e => e.status === 'approved' && (
                  ...
                  <div key={entry.id} className="bg-[#13131a]/40 p-4 rounded-lg border border-[#2a2a35] flex justify-between items-start">
                    <div>
                      <p className="font-bold text-lg text-white">{entry.client_name}</p>
                      <div className="flex flex-wrap gap-x-4 text-xs text-[#8888a0] mt-1">
                        {entry.client_email && <span>{entry.client_email}</span>}
                        {entry.client_phone && <span>{entry.client_phone}</span>}
                      </div>
                      <p className="text-sm text-[#b8b8c2] mt-2 italic">"{entry.reason}"</p>
                      <p className="text-[10px] text-[#8888a0] mt-2 uppercase tracking-widest font-bold">Approved on {new Date(entry.created_at).toLocaleDateString()}</p>
                    </div>
                    <button 
                      onClick={() => {
                      ...
                      className="text-[#8888a0] hover:text-red-500 p-2 transition-colors"
                ...
                <p className="text-[#8888a0] text-center py-10">No active DNS entries found.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="card-base !p-6 animate-fade-in">
           ...
           {reportingMetrics.confirmedCount > 0 ? (
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="card-base !p-6 flex items-center gap-4 !bg-[#0f0f12]/50"><DollarSign className="w-10 h-10 text-green-500" /><div><p className="text-[#b8b8c2] text-sm">Total Revenue</p><p className="text-3xl font-bold text-white">${(reportingMetrics.totalRevenue || 0).toFixed(2)}</p></div></div>
                  <div className="card-base !p-6 flex items-center gap-4 !bg-[#0f0f12]/50"><CreditCard className="w-10 h-10 text-[#e6398a]" /><div><p className="text-[#b8b8c2] text-sm">Total Deposits Paid</p><p className="text-3xl font-bold text-white">${(reportingMetrics.totalDeposits || 0).toFixed(2)}</p></div></div>
                  <div className="card-base !p-6 flex items-center gap-4 !bg-[#0f0f12]/50"><CheckCircle className="w-10 h-10 text-blue-500" /><div><p className="text-[#b8b8c2] text-sm">Confirmed Bookings</p><p className="text-3xl font-bold text-white">{reportingMetrics.confirmedCount}</p></div></div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="card-base !p-6 !bg-[#0f0f12]/50">
                    ...
                    {reportingMetrics.performerBookings.map(([name, count]) => (
                      <div key={name}>
                        <div className="flex justify-between items-center text-sm mb-1">
                          ...
                          <span className="text-[#b8b8c2]">{count} booking{count !== 1 ? 's' : ''}</span>
                        </div>
                         <div className="w-full bg-[#2a2a35]/50 rounded-full h-2.5">
                    ...
                    {reportingMetrics.serviceCategoryCounts.map(([category, count]) => (
                      <div key={category}>
                        <div className="flex justify-between items-center text-sm mb-1">
                          ...
                          <span className="text-[#b8b8c2]">{count} time{count !== 1 ? 's' : ''} booked</span>
                        </div>
                         <div className="w-full bg-[#2a2a35]/50 rounded-full h-2.5">
             ...
             <p className="text-[#8888a0] text-center py-10">No confirmed bookings yet to generate a report.</p>
           )}
        </div>
      )}
      ...
      <div className="card-base !p-6">
        ...
        <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-start">
                <div className="flex-grow">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                      ...
                      <span className="hidden sm:inline text-[#8888a0]">&bull;</span>
                      <p className="text-white">{booking.client_name}</p>
                  </div>
                  
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-[#b8b8c2] mb-2">
                  ...
                  <div className="text-sm text-[#b8b8c2] flex flex-wrap gap-x-4 mb-2">
                  ...
                  {booking.client_message && (
                    <p className="text-sm text-[#b8b8c2] mt-1 italic">Note: "{booking.client_message}"</p>
                  )}
                  <p className={`font-semibold capitalize text-sm mt-1`}>{booking.status.replace(/_/g, ' ')}</p>
                </div>
                <div className="text-sm text-[#b8b8c2] mt-4 md:mt-0 md:text-right flex-shrink-0">
                    ...
                    {booking.status === 'pending_deposit_confirmation' && (
                     ...
                     {booking.status !== 'confirmed' && (
                        <div className="flex items-center gap-2 pl-2 border-l border-[#2a2a35]">
                           <p className="text-xs font-semibold text-[#b8b8c2]">Admin Override:</p>
                     ...
                     <button onClick={() => handleOpenChat(booking)} className="text-xs bg-[#1a1a22] hover:bg-[#2a2a35] text-white font-bold py-1.5 px-3 rounded flex items-center justify-center gap-1.5">
                        <MessageSquare size={14}/> Chat
                     </button>
                </div>
                 <div className="flex flex-wrap gap-2 items-center">
                       <div className="relative group">
                          <RefreshCcw className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#b8b8c2]" />
                          <select 
                            value={booking.performer_id}
                            onChange={(e) => onAdminChangePerformer(booking.id, Number(e.target.value))}
                            className="input-base !py-1.5 !pl-9 !pr-8 !text-xs !w-auto appearance-none bg-[#2a2a35] hover:bg-[#2a2a35]"
          ...
          )}) : <p className="text-[#b8b8c2] text-center py-4">No bookings match the current filter.</p>}
        </div>
      </div>
      )}
        ...
        <div className="card-base !p-0">
            <h2 className="text-2xl font-semibold text-white mb-4 p-6">Payment Tracking</h2>
             <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-[#b8b8c2]">
                    <thead className="text-xs text-[#b8b8c2] uppercase bg-[#13131a]/50">
                        <tr>
                    ...
                    <tbody>
                        {paymentRelatedBookings.length > 0 ? paymentRelatedBookings.map(booking => {
                            ...
                            return (
                                <tr key={booking.id} className="border-b border-[#2a2a35] hover:bg-[#1a1a22]/50">
                                    <td className="px-3 py-4 sm:px-6 font-medium text-white whitespace-nowrap">
                                      {booking.client_name}
                                      <div className="text-[10px] text-[#8888a0] mt-1">{new Date(booking.event_date).toLocaleDateString()}</div>
                                    </td>
                                        ...
                                        <div className="flex flex-col gap-2">
                                          ...
                                          {booking.status === 'confirmed' && booking.verified_at && (
                                              <div className="text-xs text-green-300/80">
                                                  ...
                                                  <p className="text-[#8888a0]">{new Date(booking.verified_at).toLocaleString()}</p>
                                              </div>
                                          )}
                                          {booking.status === 'rejected' && (
                                            <span className="text-xs text-red-500">Rejected</span>
                                          )}
                                          <button onClick={() => handleOpenChat(booking)} className="text-[10px] bg-[#1a1a22] hover:bg-[#2a2a35] text-white font-bold py-1 px-3 rounded flex items-center justify-center gap-1 mt-1">
                             ...
                             <tr>
                                <td colSpan={6} className="text-center py-8 text-[#8888a0]">No bookings match the current filter in payment tracking.</td>
                             </tr>
                        )}
                    </tbody>
                </table>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="card-base !p-6">
             ...
             <ul className="space-y-2 max-h-60 overflow-y-auto">
                {performers.map(p => (
                    <li key={p.id} className="flex justify-between items-center bg-[#13131a]/70 p-3 rounded-md border border-[#2a2a35]/50">
                        <span className="text-white font-medium">{p.name}</span>
                        <span className={`capitalize px-2 py-1 text-xs font-semibold rounded-full ${p.status === 'available' ? 'bg-green-500/20 text-green-300' : p.status === 'busy' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-[#2a2a35]/20 text-[#b8b8c2]'}`}>{p.status}</span>
                    </li>
                ))}
             </ul>
          </div>
