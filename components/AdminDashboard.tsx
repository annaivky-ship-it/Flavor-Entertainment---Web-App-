...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ bookings, performers, doNotServeList, communications, onUpdateBookingStatus, onUpdateDoNotServeStatus, onViewDoNotServe, onAdminDecisionForPerformer, onAdminChangePerformer, onUpdatePerformer, onCreatePerformer }) => {
  ...
  return (
    <div className="animate-fade-in space-y-8">
      ...
      {/* Tab Navigation */}
      <div id="tour-booking-queue" className="border-b border-zinc-800 overflow-x-auto scrollbar-hide">
            ...
            <div className="card-base !p-6 border-orange-500/30 bg-orange-500/5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm text-zinc-400">Service Areas</label>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {SERVICE_AREAS.map(area => (
                      <label key={area} className="flex items-center gap-2 p-2 rounded bg-zinc-800 border border-zinc-700 cursor-pointer hover:bg-zinc-700 transition-colors">
                        <input
                          type="checkbox"
                          checked={performerForm.service_areas.includes(area as any)}
                          onChange={e => {
                            const current = performerForm.service_areas;
                            if (e.target.checked) {
                              setPerformerForm({ ...performerForm, service_areas: [...current, area as any] });
                            } else {
                              setPerformerForm({ ...performerForm, service_areas: current.filter(a => a !== area) });
                            }
                          }}
                          className="w-4 h-4 rounded border-zinc-600 text-orange-500 focus:ring-orange-500/20 bg-zinc-900"
                        />
