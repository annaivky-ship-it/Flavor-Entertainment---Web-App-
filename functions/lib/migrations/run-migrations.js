...
const refreshToken = (_a = config.tokens) === null || _a === void 0 ? void 0 : _a.refresh_token;
if (!refreshToken) {
        ...
        if (/^[0-9a-f]+$/.test(docId) && docId.length % 2 === 0) {
            try {
                ...
                if (decodedEmail.includes('@') && decodedEmail.includes('.')) {
                    const sha256Hash = (0, crypto_1.createHash)('sha256').update(decodedEmail.toLowerCase()).digest('hex');
                    // Create new doc with SHA-256 hash as ID
                    const newRef = db.collection('blacklist').doc(sha256Hash);
                    batch.set(newRef, Object.assign(Object.assign({}, doc.data()), { migrated_from: docId, migrated_at: admin.firestore.FieldValue.serverTimestamp() }));
