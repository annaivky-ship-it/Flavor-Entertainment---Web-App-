...
export async function handleMonoovaWebhook(req: any, res: any): Promise<void> {
  // Only allow POST
  if (req.method !== 'POST') {
  ...
  const isPaymentEvent = SUPPORTED_EVENT_TYPES.some(
    (t) => parsed.eventType.toLowerCase() === t.toLowerCase()
  );
  if (!isPaymentEvent) {
