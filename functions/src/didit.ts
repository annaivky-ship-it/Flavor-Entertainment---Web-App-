...
const DIDIT_APP_URL = process.env.DIDIT_APP_URL || 'https://theprivatebook.au';
 ...
 */
export async function processKycResult(webhookData: DiditWebhookPayload): Promise<{
    bookingId: string;
    kycResult: 'PASS' | 'FAIL';
    newStatus: string;
}> {
    ...
    const kycResult: 'PASS' | 'FAIL' = status === 'Approved' ? 'PASS' : 'FAIL';
    ...
    if (kycResult === 'PASS') {
        ...
        if (hasAmlHits) {
            ...
            const { dnsLookup } = await import('./dns/index');
            const isBlocked = await dnsLookup(
                booking.client_email_hash,
                booking.client_phone_hash
            );
